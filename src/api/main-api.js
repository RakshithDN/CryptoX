...
// Main crypto listing (Top 10 coins)
...
// Global crypto stats for About section (like total coins, exchanges, etc.)
export const getGlobalData = async () => {
  const cacheKey = 'globalData';

  const fetchGlobalData = async () => {
    try {
      const globalUrl = createUrl('/global', { x_cg_demo_api_key: apiKey });
      const exchangesUrl = createUrl('/exchanges', { x_cg_demo_api_key: apiKey });
...
// Detailed coin info for coin page or modal
export const getCoinInfo = async (coinId) => {
  const cacheKey = `coin_${coinId}`;

  const fetchCoinInfo = async () => {
    try {
      const url = createUrl(`/coins/${coinId}`, { localization: false, x_cg_demo_api_key: apiKey });
      const response = await fetch(url.toString(), API_CONFIG.DEFAULT_OPTIONS);
      const data = await handleApiResponse(response);
      ...
      return {
        ...
        totalSupply: data.market_data.circulating_supply.toLocaleString("en-US") || "Unknown",
        maxSupply,
...
export const searchCoins = async (query) => {
  ...
  const fetchSearchResults = async () => {
    try {
      await new Promise(resolve => setTimeout(resolve, 300)); // debounce-like behavior
...
// Chart endpoint to get historical prices
export const getCoinHistoricalData = async (coinId, days = '7', vs_currency = 'usd') => {
  ...
  const cacheDuration = CACHE_DURATIONS[days] || 60000;
